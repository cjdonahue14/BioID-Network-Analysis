...
BioIDUpAllandGFP$Prey->Genes3
BioIDUpAllandGFP$`Saint Score`->Prize3
names(Prize3)<-Genes3
Prize3
library(ndexr)
...
read_excel("HIPPIE2022_ConfR.xlsx")->HIPX
#load the corrected version with NFX1/NXF1 mix-up interactions removed (personal communication from BioGRID)
read_excel("HIPPIE2022_ConfR.xlsx")->HIPX
View(HIPX)
as.data.frame(HIPX)->HIPXDF
construct_interactome(HIPXDF)->HIPNetX
subnet7.3.1<-PCSF(HIPNetX,Prize3,w=10,b=2,mu=0.01)
plot.PCSF(subnet7.3.1)
...
library(tidyr)
Nodeattssep<- spread(AttsOnly,"VPInter","Hit Ratio",fill = 0)
View(Nodeattssep)
...
AttL<-match(V(subnet7.3.1)$name,LScores[,1])
V(subnet7.3.1)$LInterN<-as.numeric(LScores[,2][AttL])
#Test if function worked
get.vertex.attribute(subnet7.3.1)
vertex_attr(subnet7.3.1,name="LInterN",index = "HSPA8")
#Add NP Scores as attributes
NPScores[,1]<-sapply(strsplit(as.character(NPScores[,1]),split = " "),"[[",1)
AttNP<-match(V(subnet7.3.1)$name,NPScores[,1])
V(subnet7.3.1)$NPInterN<-as.numeric(NPScores[,2][AttNP])
#Add VP35 Scores as attributes
VP35Scores[,1]<-sapply(strsplit(as.character(VP35Scores[,1]),split = " "),"[[",1)
AttVP35<-match(V(subnet7.3.1)$name,VP35Scores[,1])
V(subnet7.3.1)$VP35InterN<-as.numeric(VP35Scores[,2][AttVP35])
#Add VP40 Scores as attributes
VP40Scores[,1]<-sapply(strsplit(as.character(VP40Scores[,1]),split = " "),"[[",1)
AttVP40<-match(V(subnet7.3.1)$name,VP40Scores[,1])
V(subnet7.3.1)$VP40InterN<-as.numeric(VP40Scores[,2][AttVP40])
#Add VP24 Scores as attributes
VP24Scores[,1]<-sapply(strsplit(as.character(VP24Scores[,1]),split = " "),"[[",1)
AttVP24<-match(V(subnet7.3.1)$name,VP24Scores[,1])
V(subnet7.3.1)$VP24InterN<-as.numeric(VP24Scores[,2][AttVP24])
#Add VP30 Scores as attributes  
VP30Scores[,1]<-sapply(strsplit(as.character(VP30Scores[,1]),split = " "),"[[",1)
AttVP30<-match(V(subnet7.3.1)$name,VP30Scores[,1])
V(subnet7.3.1)$VP30InterN<-as.numeric(VP30Scores[,2][AttVP30])
get.vertex.attribute(subnet7.3.1)
#Load resulting network into Cytoscape for aesthetic/ network graphing
createNetworkFromIgraph(subnet7.3.1,"BioID_AttUpdated_wHIPPIE22Corr")
...
cluster_edge_betweenness(subnet7.3.1)->Cl731
data.frame(Membership=Cl731$membership, Genes=Cl731$names)
x <- data.frame(Membership=Cl731$membership, Genes=Cl731$names) %>% dplyr::arrange(Membership)
unique(x$Membership)
